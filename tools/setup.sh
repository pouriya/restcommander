...
_print_usage(){
  echo "Usage:"
  echo "    <SCRIPT> <RESTCOMMANDER_EXECUTABLE> <ROOT_DIRECTORY> <TEMPLATE_ROOT_DIRECTORY>"
}
...
if [ -f "${ROOT_DIR}/bin/restcommander" ]
then
  echo "${ROOT_DIR}/bin/restcommander already exists!"
  echo "Make a backup and remove it and try again"
  exit 1
fi
mkdir -p "${ROOT_DIR}/bin"
cp "${EXE}" "${ROOT_DIR}/bin/restcommander"
EXE_NAME="$(basename ${EXE})"
EXE="${ROOT_DIR}/bin/restcommander"
TEMPLATE_EXE="${TEMPLATE_ROOT_DIR}/bin/restcommander"
chmod a+x "${EXE}"
...
WWW_DIR="${ROOT_DIR}/srv/restcommander/www"
TEMPLATE_WWW_DIR="${TEMPLATE_ROOT_DIR}/srv/restcommander/www"
mkdir -p "${WWW_DIR}"
_replace_toml_value_in_file "static_directory" "static_directory" "\"${TEMPLATE_WWW_DIR}\"" "${EXAMPLE_CONFIG_FILE}"
SCRIPTS_DIR="${ROOT_DIR}/srv/restcommander/scripts"
TEMPLATE_SCRIPTS_DIR="${TEMPLATE_ROOT_DIR}/srv/restcommander/scripts"
mkdir -p "${SCRIPTS_DIR}"
if [ ! "$(ls -A ${SCRIPTS_DIR})" ]
then
...
LOG_DIR="${ROOT_DIR}/var/log/restcommander"
TEMPLATE_LOG_DIR="${TEMPLATE_ROOT_DIR}/var/log/restcommander"
mkdir -p "${LOG_DIR}"
if [ ! "$(ls -A ${LOG_DIR})" ]
then
...
DATA_DIR="${ROOT_DIR}/var/lib/restcommander"
TEMPLATE_DATA_DIR="${TEMPLATE_ROOT_DIR}/var/lib/restcommander"
mkdir -p "${DATA_DIR}"
